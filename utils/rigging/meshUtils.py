...
class Api_mesh(OMapi.API_base):
    # 选中根骨骼 创建出一条简模
    def buid_meshJM(self , joint, JMwidth, direction , headOffect = False ,smooth = True):
    ...
    #选中点拷贝出模型
    #输入的的是列表
    def copyMesh(self , pointList):

        if "." in pointList[0]:
            vtxList = []
            old_mesh_name = pointList[0].split('.')[0]
            print pointList
            new_mesh_Name = cmds.duplicate(old_mesh_name)

            # 将点序列转化为新模型的点序列
            for vtx in pointList:
                # new_mesh_Name = vtx.split(".")[0]
                vtx = vtx.replace(old_mesh_name, new_mesh_Name[0])
                vtxList.append(vtx)
            ...
            new_mesh_numface = cmds.polyEvaluate(new_mesh_Name[0], face=True)
            ...
            for a in list(setfirst):
                listFace.append(new_mesh_Name[0] + '.f[' + str(a) + ']')
    ...
    #合并模型
    #meshList 是列表
    def combneMesh(self,meshList):
        if not cmds.objExists('copyWeightMesh_mid_temp'):
            mesh = cmds.polyUnite(meshList, n='copyWeightMesh_mid_temp' ,ch = 0 )
        else:
            cmds.delete('copyWeightMesh_mid_temp')
            mesh = cmds.polyUnite(meshList, n='copyWeightMesh_mid_temp', ch=0)
        cmds.delete(meshList)
        return mesh
